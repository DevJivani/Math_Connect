...
//       { otp: otp.toString(), otpExpires, email },
...
const login = async (req, res, next) => {
  ...
  console.log("Generated JWT Token for user:", existingUser.email);
...
const sendOtpEmail = async (req, res) => {
  console.log('sendOtpEmail called with email:', req.body.email);
  ...
  try {
    ...
    console.error('Server error in sendOtpEmail:', error);
...
const verifyOtp = async (req, res) => {
  console.log('verifyOtp called with:', { 
    ...
    otpLength: req.body.otp ? req.body.otp.length : 0,
    hasToken: !!req.body.otpToken 
  });
  ...
  try {
    ...
    console.log('Decoded token:', { 
      email: decoded.email, 
      otp: decoded.otp ? 'Present' : 'Missing',
      expires: new Date(decoded.otpExpires).toLocaleString()
    });

    // Check if the email matches the one stored in the token
    if (decoded.email !== email) {
      console.log('Email mismatch:', decoded.email, '!=', email);
      return res.status(400).json({ message: 'Invalid email or OTP.' });
    }

    // Check if OTP matches and is not expired
    if (decoded.otp !== otp) {
      console.log('OTP mismatch:', decoded.otp, '!=', otp);
      return res.status(400).json({ message: 'Invalid OTP.' });
    }

    if (Date.now() > decoded.otpExpires) {
...
const resetPassword = async (req, res) => {
  console.log('resetPassword called for email:', req.body.email);
